...
public class ConnectorWasm3 : ConnectorAbstract
{
    ...
    public override void Load(int sandboxId, string url)
    {
        ...
        req.SendWebRequest().completed += operation =>
        {
            ...
            var environment = new Wasm3DotNet.Wrapper.Environment();
            runtime = new Runtime(environment);
            var module = runtime.ParseModule(wasm);

            module.LinkRawFunction("proto", "_callEngine32", "v(iiii)", LogInt);
    ...
    static IntPtr CallEngine32(IntPtr runtime, IntPtr ctx, IntPtr sp, IntPtr mem)
    {
        // GameObject.CreatePrimitive(PrimitiveType.Capsule);
        /*
        Debug.Log("LogInt: " + value.ToString());
